 ...
 * - Updates links in StudentGuide.md to be relative to the class repo it's
 copied to.
...
const options = {
  ...
  get dayFixed() {
    // parseInt incase input is full directory name e.g. "01-Day"
    if (isNaN(this.day / 1)) {
      logger.warn(`this.day: "${this.day}" only requires day number`)
    }
    return pad(parseInt(this.day));
  },
  get weekFixed() {
    // parseInt incase input is full directory name e.g. "20-react"
    if (isNaN(this.week / 1)) {
      logger.warn(`this.week: "${this.week}" only requires week number`)
    }
    return pad(parseInt(this.week));
  },
  ...
  get srcLessonPlans() {
    return userHome + '/' + pkgFile.paths.sourceRepoDir +
        pkgFile.paths.lessonPlanDir + this.srcWeekDirName;
  },
  get srcClassContents() {
    return userHome + '/' + pkgFile.paths.sourceRepoDir +
        pkgFile.paths.classContentDir + options.loadedCohort.content_dir;
  },
  get srcHomeworkDir() {
  ...
  get srcSupplementalDir() {
    // DONE: RegExp for finding supplemental directory can be "03-Supplemental"
    // or just "Supplemental" so need to match both.
    const folderName = findFileOrDir(
        this.srcClassContents, new RegExp(pkgFile.paths.supplementalDir));
    // If no folder name found return false since we don't want to copy
    // anything.
    if (!folderName) {
  ...
  get dist() {
    return userHome + '/' + pkgFile.paths.cohorts[this.cohort];
  },
  get distWeek() {
      ...
      (filePath) => {
        ...
        fs.readFile(filePath, 'utf8', function(err, data) {
          ...
          var regexString = '(\\.\\.\\/\\.\\.\\/\\.\\.\\/01-Class-Content\\/' +
              options.loadedCohort.content_dir + '(\\d+-Homework)\\/)';
          logger.debug(regexString)
          var regex = new RegExp(regexString, 'gm');
          result = data.replace(regex, function(match, origUrl) {
            logger.info(
                'Rename directory: \n' +
                'OLD: ' + origUrl + '\n' +
              ...
              function(match, origUrl, activity, offset, inputString) {
                ...
                for (let day in activities) {
                  ...
                  if (activities[day].includes(activity)) {
                    ...
                    let newUrl = options.distClassContentDirName + '/' +
...
/**
 * Copies files (defined as keys in dirsToCopy) to their day folder destination.
 * The values of dirsToCopy represent the new file names (if changed).
 *
 * @param Object day
 */
...
function makeDirectories(activities) {
  ...
  copyFile(options.srcSupplementalDir, options.distSupplementalDir);
  // ## add .gitignore to ignore 'Solutions' in Homework dir (remove this after
  // 1 week of class passes)
  ...
  createGitIgnore(
      options.distWeekHomeworkDir,
      findFileOrDir(options.distWeekHomeworkDir, /Solutions?/));
  // ## Copy supplemental folder in the week.
  // TODO: organize the supplemental directory logic
  copyFile(
      options.srcLessonPlans + 'Supplemental',
      options.distWeek + 'Supplemental');

  // ## Copy class-day activities for week.
  const dayConfig = {
    day01: {distDir: options.day01.fullPath, srcDir: '01-Day'},
    day02: {distDir: options.day02.fullPath, srcDir: '02-Day'},
    day03: {distDir: options.day03.fullPath, srcDir: '03-Day'}
  };
  for (let day in dayConfig) {
    switch (true) {
      // Only scan the filesystem when there's configured activities in the JSON
      // for the day.
      case options.loadedCohort.activities[day].length === 0:
        logger.error(
            '\n' + day +
            ' has no configured activities!\nSee ./config/curriculum.json');
        continue;
      // --day option was passed in so limit filesystem activity to only the
      // passed in day.
    ...
    // ## add .gitignore to ignore "Solved" directories for days activities
    // (remove after class)
...
// Assign cohort curriculum data for the given week.This will be used to pull
// out the weekly content.
...
makeDirectories(options.loadedCohort.activities);
